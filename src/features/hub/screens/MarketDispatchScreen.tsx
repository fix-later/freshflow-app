...
const STATUS = {
  ...
  on_route: { label: 'Đang đi chợ', color: Colors.secondary, background: '#E8F4FC' },
  at_hub: { label: 'Đã về Hub', color: Colors.success, background: Colors.successLight },
} as const;
      ...
      <View style={styles.screen}>
        <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.sectionHeading}>
            ...
            <View style={styles.assignedBadge}><Text style={styles.assignedBadgeText}>{summary.assignedCount}/{dispatches.length}</Text></View>
          </View>
        ...
        <View style={styles.footer}>
          <View style={styles.footerCopy}>
            ...
            <Text style={styles.footerValue}>{summary.totalKg.toLocaleString('vi-VN')} kg</Text>
          </View>
  ...
  return (
    <View style={styles.dispatchCard}>
      <View style={styles.cardHeader}>
        <View style={styles.marketIcon}><Ionicons name="storefront-outline" size={21} color={Colors.primary} /></View>
      ...
      <View style={styles.loadHeader}>
        ...
        <Text style={styles.loadValue}>{dispatch.fillRate}%</Text>
      </View>
...
function Summary({ value, label }: { value: string; label: string }) {
  return <View style={styles.summaryItem}><Text numberOfLines={1} adjustsFontSizeToFit style={styles.summaryValue}>{value}</Text><Text style={styles.summaryLabel}>{label}</Text></View>;
}

function Demand({ icon, value, label }: { icon: keyof typeof Ionicons.glyphMap; value: string; label: string }) {
  return <View style={styles.demandItem}><Ionicons name={icon} size={17} color={Colors.primary} /><Text style={styles.demandValue}>{value}</Text><Text style={styles.demandLabel}>{label}</Text></View>;
}
...
function AssignmentRow({ icon, label, value, editable, onPress }: {
  ...
  return (
    <View style={styles.assignmentRow}>
      <View style={styles.assignmentIcon}><Ionicons name={icon} size={18} color={Colors.primary} /></View>
      ...
      {editable ? (
        <Pressable accessibilityLabel={`Đổi ${label.toLowerCase()}`} style={styles.changeButton} onPress={onPress}>
          <Text style={styles.changeText}>Đổi</Text><Ionicons name="chevron-forward" size={15} color={Colors.primary} />
        </Pressable>
          ...
          <ScrollView style={styles.optionList} contentContainerStyle={styles.optionListContent} showsVerticalScrollIndicator={false}>
            {target && isVehicle && MARKET_VEHICLES.map((vehicle) => {
              ...
              return (
                ...
                >
                  <View style={styles.optionIcon}><Ionicons name="car-outline" size={20} color={vehicle.available ? Colors.primary : Colors.textMuted} /></View>
                  ...
                  {!vehicle.available ? <Text style={styles.unavailableText}>Bảo trì</Text> : selected ? <Ionicons name="checkmark-circle" size={22} color={Colors.primary} /> : <Ionicons name="ellipse-outline" size={22} color={Colors.outline} />}
                </Pressable>
              );
            })}
            ...
            {target && !isVehicle && MARKET_DRIVERS.map((driver) => {
              ...
              return (
                <Pressable key={driver.id} style={styles.optionRow} onPress={() => onSelectDriver(target.dispatchId, driver.name)}>
                  <View style={styles.optionIcon}><Ionicons name="person-outline" size={20} color={Colors.primary} /></View>
                  ...
                  {selected ? <Ionicons name="checkmark-circle" size={22} color={Colors.primary} /> : <Ionicons name="ellipse-outline" size={22} color={Colors.outline} />}
                </Pressable>
              );
            })}
          </ScrollView>
...
const styles = StyleSheet.create({
  ...
  summaryBand: { backgroundColor: Colors.primary, paddingHorizontal: 16, paddingTop: 18, paddingBottom: 20 },
  eyebrow: { color: 'rgba(255,255,255,0.72)', fontSize: 10, fontWeight: '800' },
  summaryTitle: { color: Colors.onPrimary, fontSize: 20, fontWeight: '800', marginTop: 5 },
  ...
  summaryValue: { color: Colors.onPrimary, fontSize: 14, fontWeight: '800', maxWidth: '100%' },
  summaryLabel: { color: 'rgba(255,255,255,0.68)', fontSize: 8, marginTop: 3 },
  notice: { marginHorizontal: 16, marginTop: 16, padding: 11, borderRadius: 8, backgroundColor: Colors.successLight, flexDirection: 'row', alignItems: 'flex-start', gap: 9 },
  ...
  assignedBadge: { minWidth: 34, height: 28, borderRadius: 7, backgroundColor: Colors.successLight, alignItems: 'center', justifyContent: 'center' },
  assignedBadgeText: { color: Colors.primary, fontSize: 11, fontWeight: '800' },
  dispatchList: { marginHorizontal: 16, gap: 10 },
  dispatchCard: { borderWidth: 1, borderColor: Colors.outlineVariant, borderRadius: 8, backgroundColor: Colors.surfaceContainerLowest, padding: 12 },
  cardHeader: { flexDirection: 'row', alignItems: 'center' },
  marketIcon: { width: 38, height: 38, borderRadius: 8, backgroundColor: Colors.successLight, alignItems: 'center', justifyContent: 'center' },
  ...
  demandValue: { color: Colors.textPrimary, fontSize: 11, fontWeight: '800', marginTop: 4 },
  ...
  assignmentIcon: { width: 30, height: 30, borderRadius: 7, backgroundColor: Colors.successLight, alignItems: 'center', justifyContent: 'center' },
  ...
  changeButton: { height: 30, paddingHorizontal: 8, borderRadius: 6, flexDirection: 'row', alignItems: 'center', backgroundColor: Colors.successLight },
  changeText: { color: Colors.primary, fontSize: 10, fontWeight: '800' },
  ...
  loadValue: { fontSize: 10, fontWeight: '800', color: Colors.primary },
  ...
  footer: { minHeight: 70, paddingHorizontal: 16, paddingVertical: 10, borderTopWidth: 1, borderTopColor: Colors.outlineVariant, backgroundColor: Colors.surfaceContainerLowest, flexDirection: 'row', alignItems: 'center', gap: 12 },
  ...
  confirmButton: { flex: 1, minWidth: 0, height: 46, borderRadius: 8, backgroundColor: Colors.primary, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 7 },
  confirmButtonText: { color: Colors.onPrimary, fontSize: 11, fontWeight: '800' },
  modalRoot: { flex: 1, justifyContent: 'flex-end' },
  modalBackdrop: { ...StyleSheet.absoluteFillObject, backgroundColor: 'rgba(16,24,18,0.48)' },
  modalSheet: { height: '65%', maxHeight: 520, backgroundColor: Colors.surfaceContainerLowest, borderTopLeftRadius: 12, borderTopRightRadius: 12, paddingBottom: 18 },
  ...
  optionIcon: { width: 36, height: 36, borderRadius: 8, backgroundColor: Colors.successLight, alignItems: 'center', justifyContent: 'center' },
