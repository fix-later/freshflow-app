...
import { MOCK_HUB, MOCK_STOPS, type MockStop } from '../mockData';
    ...
    >
      ...
      <View style={styles.cardInfo}>
        ...
        <Text style={styles.cardMeta}>
          {stop.items.length} loại · {totalQty} đơn vị
        </Text>
      </View>
...
export function PickupConfirmScreen({ route, navigation }: Props) {
  ...
  // TODO: replace with API: await driverApi.getPickupList(routeId)
  const hub = MOCK_HUB;
  ...
  const allChecked = checked.size === stops.length;
  const totalItems = stops.reduce((s, stop) => s + stop.items.reduce((a, i) => a + i.quantity, 0), 0);
          ...
          <>
            ...
            <View style={styles.progressRow}>
              ...
              <Text style={styles.progressCount}>
                <Text style={styles.progressHighlight}>{checked.size}</Text>/{stops.length} kiện
              </Text>
            </View>
        ...
        <View style={styles.footerInfo}>
          <Text style={styles.footerLabel}>
            {allChecked
              ...
              : `Còn ${stops.length - checked.size} kiện chưa kiểm tra`}
          </Text>
          <Text style={styles.footerSub}>{stops.length} điểm · {totalItems} đơn vị</Text>
        </View>
        <Pressable
